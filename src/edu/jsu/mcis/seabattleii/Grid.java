    ...
    public void deployShip(Ship s) { // INSERT YOUR CODE HERE
        ...
        while ( !empty ) {
            ...
            if(alignment == Ship.HORIZONTAL){
                ...
                col = (int) (Math.random() * (GRID_SIZE - shipSize));
                
            }
            /* Ship will be vertical; select row within range! */
            else if(alignment == Ship.VERTICAL){
                row = (int) (Math.random() * (GRID_SIZE - shipSize));
            ...
            for(int i = 0; i < shipSize; ++i){
                ...
                if(alignment == Ship.HORIZONTAL){
                    square = grid.get(row).get(col + i);
                }

                /* If vertical, check for empty space in next row */     
                else if(alignment == Ship.VERTICAL){
                    square = grid.get(row + i).get(col);
                }
                
                /* Check to see if that square is empty */
        ...
        for(int i = 0; i < shipSize; ++i){
            square = new ShipSquare(s,i);
            if(alignment == Ship.HORIZONTAL){
                grid.get(row).set(col + i, square);
            }
            else if(alignment == Ship.VERTICAL){
                grid.get(row + i).set(col,square);
            }
        }
